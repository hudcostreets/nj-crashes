...
def map_df(v: pd.DataFrame) -> pd.DataFrame:
    ...
    # note. `normalize` joins the vehicle's raw (year, cc, mc, case) to the crash's
    # raw `mc_dot`; remapping mc → geocoded (2026 v2 PR) broke that and orphaned
    # ~14% of vehicles (every geocoded crash). crash_id is the only key kept.
