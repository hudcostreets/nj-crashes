...
Phase 1 produces, under {out_dir} (default `data/cells/`):

    raw/h3_r{base_res}/{shard_cell}.parquet   # one per r{shard_res} parent
    manifest.json

`shard_cell` is rendered as the H3 cell's hex string at `shard_res` (default
r4 — NJ has ~10–15 non-empty r4 cells). Within each shard, rows are sorted
by `h3_r{base_res}` (int64) so parquet row-group min/max statistics give the
worker tree-structured pruning at any coarser N.
