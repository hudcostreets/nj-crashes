...
2. **hudcostreets → HCCS ✓ (2026-09-13)** — CNAME re-pointed to `crashes.pages.dev` in Squarespace DNS (user, via passkey), then `crashes.hudcostreets.org` removed from RAC `nj-crashes` Pages → HCCS `crashes` validated + went active. **Handoff blip ~2m25s** (16:38:11 RAC release → 16:40:36 HCCS active; HTTP 409 in between): CF forbids the hostname being active on two Pages projects, so HCCS can't pre-warm — the gap is unavoidable. `crashes.hccs.dev` unaffected throughout. RAC `nj-crashes` now has **zero custom domains** (still reachable at `nj-crashes.pages.dev`; project not yet deleted).
