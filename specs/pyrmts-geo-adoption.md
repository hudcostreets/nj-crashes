...
- Remaining step-1 sequencing (crashes-side): re-pin durable (`pds gh`-style) → commit the swap → deploy the worker via `cells-api/deploy.dvc` → spot-check `/v1/cells` at a street zoom.
