...
export function CrashMapSection({
    ...
    const cellPxTarget = useMemo(() => {
        ...
        // Statewide overview is height-bound (NJ is tall/narrow), so the
        // surplus viewport around it is empty ocean/PA letterbox that
        // shouldn't spend bin budget — widening it otherwise steps the
        // pick coarser over the identical map. Clip `areaPx` to the state
        // bbox (∩ viewport). Applies to full-screen too: `clippedAreaPx`
        // is self-limiting — once zoomed inside NJ the ring covers the
        // whole viewport and the clip is a no-op — so it only refines the
        // zoomed-out overview and never touches the zoomed-in payload cliff.
